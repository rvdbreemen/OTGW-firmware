...
> ADR-123 (concurrency-model) en ADR-128 (stop ESP8266 support).
...
Dat loopt via een nieuw besluit-ADR (**ADR-128**, Proposed), niet door die Accepted
...
klus ná acceptatie van ADR-128.
...
- **Dual-target-spanning: opgelost door ESP8266 te schrappen.** Dit was eerder de
  ...
  **ADR-128** (Proposed); daarna volgt de "alongside ESP8266"-tekst in
  README/MANUAL. De code-verwijdering zelf is een aparte, gefaseerde klus na
  acceptatie van ADR-128.
...
- ADR-123 (2.0.0 concurrency-model), **ADR-128** (stop ESP8266 support, supersedeert ADR-082)
