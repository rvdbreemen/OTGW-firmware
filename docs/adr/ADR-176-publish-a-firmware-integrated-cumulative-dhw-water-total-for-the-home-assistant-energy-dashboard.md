...
* Publish the cumulative total as a separate entity. The existing MsgID 19 rate sensor
  keeps unit `l/min` and gains `device_class: volume_flow_rate`; it must not be retyped
  as `water`.
...
* Double-count when MsgID 19 is both observed on the bus and generated by this
  firmware's own OTDirect master scheduler.
...
- [ ] On the OTDirect path, is MsgID 19 counted once or twice when this firmware is the
      master? Name the single accumulation point and how the other path is excluded.
- [ ] What is the concrete flash-write rule, and what erase-cycle budget justifies the
      chosen numbers?
...
- [ ] Is the counter user-resettable, and through which surface?
- [ ] Does the persisted counter belong in the existing settings store or in its own
