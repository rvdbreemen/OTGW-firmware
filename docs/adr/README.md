...
- **[ADR-018: ArduinoJson for Data Interchange](ADR-018-arduinojson-data-interchange.md)** *(Superseded by ADR-042)*  
  Historical decision for JSON handling (replaced by manual JSON handling due dependency removal).

- **[ADR-042: Remove ArduinoJson Dependency](ADR-042-remove-arduinojson-dependency.md)** 🆕  
  Removes ArduinoJson entirely and replaces required parsing/serialization with bounded manual JSON helpers.
