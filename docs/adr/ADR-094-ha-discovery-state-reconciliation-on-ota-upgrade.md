...
- **ADR-097**: MQTT publish gating by source and slave-echo. Affects which topics a given firmware version publishes; a future change to ADR-097's per-MsgID classification table is exactly the kind of shape transition this ADR reconciles.
