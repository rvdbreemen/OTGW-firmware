...
2. **`publishPSSummaryFieldValue()` computes `validForMaster` once** at function entry, using the global `OTlookupitem` already populated by the caller (`PROGMEM_readAnything(&OTmap[msgid], OTlookupitem)` in the caller). Each value-bearing case (`ot_f88`, `ot_s16`, `ot_u16`, `ot_s8s8`, `ot_u8u8`, `ot_u8`) gates `sendMQTTData(label, ...)` and the `OTcurrentSystemState` writes on `validForMaster`. The `ot_flag8flag8` case is untouched (status-flag semantics: per-MsgID switch already inside the case, parallel to the live-bus exception for `OT_Statusflags` / `OT_StatusVH` / `OT_SolarStorageMaster`).

3. **`setMsgLastUpdated()` remains called regardless** of `validForMaster`. The epoch tick is cosmetic (drives WebUI freshness), consistent with the live-bus path where `setMsgLastUpdated` is gated only on the broader `is_value_valid` (not on the master-topic invariant).
