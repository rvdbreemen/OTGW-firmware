...
| `button` | 1 | pseudo-ID 251 | Reset Gateway (PIC-gated) |
| `select` | 8 | pseudo-ID 251 | GPIO A/B and LED A–F function selects (PIC-gated) |
| `sensor` (Dallas) | dynamic | pseudo-ID 246 | Dallas temperature sensors (one per address) |

#### PIC Control Entities (pseudo-ID 251)

The following entities are only published when the OTGW PIC is available (`isPICEnabled()`). They appear under the OTGW device card in Home Assistant.
