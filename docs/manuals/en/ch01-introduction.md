...
- **PROGMEM MQTT discovery**: Auto-discovery payloads are now compiled into flash memory (PROGMEM) and published via an asynchronous bitmap-driven drip publisher. This replaces the previous LittleFS-based approach, eliminating large RAM staging buffers and improving heap stability.
...
- **SimpleTelnet**: The telnet debug log now uses SimpleTelnet, a unified multi-client telnet library that replaces the separate TelnetStream and ESPTelnet libraries.
