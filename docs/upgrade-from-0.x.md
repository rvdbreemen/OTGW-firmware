...
If you use OTmonitor or another ser2net client on TCP port 25238, v1.3.1 introduced proper coordination between the firmware's internal command queue and ser2net traffic. Previously, time-sync commands (SC=) and date commands (SR=) bypassed the command queue entirely, which could collide with commands sent from OTmonitor.
...
- **v1.0.0** introduced MQTT auto-discovery for the most common heating and DHW sensors.
- **v1.2.0** expanded this to cover the full OpenTherm protocol: **309 discovery configurations** across 80+ message IDs. Cooling, solar thermal, ventilation (heat recovery), CH2, operational counters, fault diagnostics -- all auto-discovered without manual YAML.
...
- **REST API reference** (`docs/api/README.md`) -- all v2 endpoints documented with examples, rate-limiting guidance, and integration samples for Home Assistant, Python, and JavaScript. An OpenAPI 3.0 specification (`docs/api/openapi.yaml`) is included for tools like Swagger UI or Postman.
- **MQTT topic reference** (`docs/api/MQTT.md`) -- full namespace documentation, birth/LWT messages, per-source topic separation (optional), and details on all 309 HA discovery configurations.
...
**Graphs (v1.0.0)**
Built-in ECharts-based visualization of boiler temperatures, setpoints, water pressure, and modulation level. Dallas temperature sensors are included in the graph with a 16-color palette. No external tools like Grafana needed for a quick overview.

**Dallas sensor labels (v1.1.0)**
...
## PIC gateway settings visibility (v1.3.0)
...
## WiFi recovery without reflashing (v1.3.0)
...
## Optional HTTP Basic Auth (v1.3.0)
...
## Webhook support (v1.2.0)

The firmware can fire an HTTP request when a status bit changes -- boiler flame on, fault detected, DHW active, etc. Useful for triggering external automations without going through MQTT.
...
- **REST API v0 and v1 removed**: These endpoints return 410 Gone. If you have scripts or integrations calling `/api/v0/...` or `/api/v1/...`, update them to `/api/v2/...`. See `docs/api/README.md` for the full reference.
- **MQTT topic spelling corrections** (v1.2.0): A handful of typos in topic names were fixed (e.g., `eletric_production` became `electric_production`). After upgrading, delete orphaned entities in Home Assistant and let discovery recreate them correctly.
