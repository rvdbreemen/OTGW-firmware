...
OTA (over-the-air) firmware update is supported after initial USB flashing: upload a new `.bin` via the web UI or `POST /update`. The LittleFS filesystem must be reflashed via USB when web assets change (OTA for filesystem is not supported on ESP8266 due to memory constraints; ESP32 has two OTA app slots for safe rollback).
