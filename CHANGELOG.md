...
- **Port 25238 accepts two clients again, without their commands getting mixed up.** v1.7.5 cut the port to one client because two writers could splice their bytes into one malformed PIC command, which left anyone running Home Assistant or Domoticz next to OTmonitor stuck. Two clients can connect again, and both receive everything the PIC sends. Toward the PIC, one client at a time holds a write floor: its bytes go through unchanged and immediately, while the other client's bytes wait untouched until the holder has been quiet for 100 ms (3 s once it has sent binary data, so a PIC upgrade from OTmonitor is never interrupted). `/api/v2/device/info` and the Device Info page now show the connected clients and the last address that was refused. Requested by Schelte Bron and iandury_. (ADR-097, TASK-1167)
