...
- ADR-093 proposed: keep the cumulative DHW water total in RAM and announce it on first data. It supersedes three requirements of ADR-090's Decision Contract that building the feature showed to be wrong: persisting the counter to flash, registering it in the boot-publish path, and keeping the rate sensor on the `l/min` unit string. It also settles a question ADR-090 did not address, namely what the integrator does when the bus falls silent. **The shipped code follows ADR-093, which is still Proposed, so it deviates from Accepted ADR-090 on those three points.** The deviation is deliberate and recorded here; ADR-093 awaits review.
