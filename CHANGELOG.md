...
- **A diagnose screen in the web interface.** The PIC can be loaded with Schelte Bron's diagnostic firmware instead of the gateway firmware, which replaces the OpenTherm protocol with a test menu on the serial line. Reaching that menu used to mean attaching a separate terminal tool. The gateway now recognises the diagnose banner and shows a PIC diagnose page: it is the home screen while that firmware is loaded, because there is no gateway telemetry to show, and it disappears again the moment a gateway PIC is detected. The menu is fetched when the screen opens, typed input goes straight to the PIC over the existing OpenTherm-log socket, and the page carries a short explanation plus a link to the diagnostic firmware page for the per-test connections. After flashing PIC firmware the interface switches to the right screen by itself. Verified on hardware across a full gateway to diagnose and back round trip, with the live menu answering. Thanks to Schelte Bron for correcting the on-screen explanation, which was accurate only for test 1. (TASK-1127)
...
- **The 5-minute status burst no longer loses its last dozen topics.** The send-buffer check added in beta.4 waited only a few scheduler turns for room, which is shorter than one acknowledgement round trip to the broker, so the tail of every burst was skipped: the `otgw-pic/settings/*` topics every 5 minutes and the end of the hourly stats. It now waits up to 200 ms for room, and after a skip it stops waiting for a second, so a broker that has stopped reading still cannot stall the gateway once per publish. OpenTherm values were not affected; measured on the bench with simulated boiler traffic. Reported by jaronbor on GH #682. (TASK-1163, TASK-1164)

- **The USB serial captures now read at 9600 baud, the rate the firmware actually uses.** Both capture scripts assumed 115200, which the firmware never sets: `OTGWSerial.cpp:836` opens the PIC link at 9600, 8N1. A capture at the wrong rate is framing garbage that looks like a broken device, and on GH #684 it cost a reporter a round trip. The shell capture also warns when most lines carry non-printable bytes, the signature of a wrong baud. (TASK-1140)

- **A publish that could not be framed now waits, then stands down, instead of starting a packet it cannot finish.** MQTT has no way to abort a packet: once a PUBLISH header with its Remaining Length is on the wire, the broker will consume exactly that many following bytes, so a half-written header left only one correct remedy, dropping the link. That remedy is what produced the clean reconnects reported after the previous fix. Every publish path now checks the TCP send buffer against the header and topic before starting, discovery configs included, gives lwIP a short non-blocking chance to drain, and defers the publish only when the room never appears. In beta.4 the discovery composers, which send the largest payloads, bypassed that check. Two counters, `mqtt_sndbuf_skips` and `mqtt_desync_drops`, are readable in `/api/v2/device/info`; until now the only way to see how often this happened was to read the broker's own log. Every desync remedy now goes through one helper, so `mqtt_desync_drops` counts all of them: in beta.4 it counted only two of thirteen sites and read low for the wrong reason. Separately, a stalled socket can no longer hold the firmware for up to ten consecutive five-second write timeouts, about 50 seconds with the PIC serial line undrained; a chunk write now gives up after one. Worth knowing when reading those figures: a short write means the socket accepted zero bytes for five consecutive seconds, because the ESP8266 timeout resets on every byte of progress, so a rising count is a network symptom rather than a tuning knob. Measured on a bench gateway against a broker that stops reading: 38 publishes deferred and the link never dropped by the desync remedy. Reading the counter: a healthy boot now shows a few dozen deferrals, about 59 on a bench gateway, because the discovery burst fills the send buffer and those configs are retried by the discovery drip until none are pending. What signals a stall is the count still climbing after boot has settled, not its absolute value. The longest the firmware stayed unresponsive against a broker that stops reading was 4.9 seconds, one write timeout, where the old retry loop allowed up to ten. (TASK-1154, TASK-1155, GH #682)

- **The debug console could drop output without anyone being able to tell.** `SimpleTelnet::write()` discarded whatever the TCP stack did not accept and returned the full byte count anyway, so a truncated console looked exactly like a quiet one. On ESP8266 a short write is the normal result once the lwIP send buffer fills, which happens when the reading client stops draining. The library now returns the count actually accepted and counts the shortfall; the two counters are readable as `telnet_tx_dropped` and `otgwstream_tx_dropped` in `/api/v2/device/info`. There is deliberately no retry: the underlying write already blocks while the reader makes progress and only comes back short after a full second with none, so a retry would either wait another second on a socket that just proved it is stuck, or never run. beta.4 shipped a bounded retry that was inert for exactly that reason; it has been removed. No fixed per-line delay was added: at the measured peak of 43 console lines per second, 1 ms per line would block the loop for 43 ms of every burst second, and the serial port is reserved for the PIC. Measured on a bench gateway a normally reading client loses nothing, loss appears only under sustained backpressure, and 16 bytes of static RAM is the whole cost. (TASK-1148, TASK-1155)

- **The first command sent on port 25238 was discarded, so Home Assistant's OpenTherm Gateway integration could not connect.** `SimpleTelnet::_drainClient()` read and threw away every pending inbound byte at accept, justified as flushing telnet negotiation. The server only accepts a client on the next `loop()` pass, so a client that pipelines a command with `connect()` had that command sitting in the receive buffer when the drain ran, and lost it. `pyotgw` writes `PS=0` about 20 ms after the TCP handshake and waits for `PS: 0`, which never came, so the integration failed with `cannot_connect` while raw OpenTherm frames streamed in on the same connection and made the port look healthy. Writing the same command about a second after connecting always worked, which is what made it look like a timing fault rather than a discard. The loop was unconditional and could not tell an IAC sequence from payload; on port 25238 there is no telnet negotiation to strip at all, so it could only destroy user data. The helper now flushes outbound only. The telnet console on port 23 is unaffected: it runs in character mode and unhandled bytes already fall through `handleDebugChar()`'s `default` case. Fixed in the SimpleTelnet submodule as a backport of upstream `a909731`, which the 2.0.0 line already carries. Reported by petrister. (GH #685, TASK-1146)

- **PIC functions stayed disabled after a PIC running non-gateway firmware was detected.** Boot-time detection can miss the PIC, and the recovery path re-enabled it from the banner, but it matched the gateway banner only. The diagnostic and interface firmwares announce themselves differently, so a gateway that had missed its PIC never recovered while either of those was loaded, and the web interface kept reporting no PIC at all. Recovery now triggers on any recognised firmware banner. (TASK-1126)

- **The boiler and thermostat entities could stay on "connected" forever after the PIC stopped talking.** Two independent defects. The liveness timeout was computed inside the OT-message processing path, so it was only ever evaluated when a frame arrived: a PIC that stopped delivering left the 30 second window unevaluated, and the flags held their last value until the next reboot. The evaluation now lives in its own function, called both when a frame arrives and from the 3 second tick, so silence is reflected as well as traffic. Separately, both publish paths were gated on PIC presence: the on-change publishes sat behind `isPICEnabled()` and the 5 minute heartbeat opened with an early return on the same condition, so a gateway whose PIC was gone published nothing on these topics at all. An absent PIC is not a reason to stay quiet about the bus, it is the reason the bus is dead, so the gate is gone for the three bus-presence values; `gateway_mode` keeps it, because that one genuinely describes the PIC. These topics are not retained, which is why the heartbeat matters: it is the only thing that refreshes them for a Home Assistant that restarted. Evaluation is suppressed while a flash is running, because a PIC update stops the stream for longer than the timeout by design and would otherwise flap every entity on each update. One further effect worth knowing: the Home Assistant climate entity reserves `hvac_mode: off` for a disconnected thermostat, but that state was only reachable while the other side of the bus kept talking. On a bus that went completely silent the entity held its last mode forever, because the code that would have set it to off was itself only reached by an arriving frame. It now reports off. Reported in Discord by tranquil_kiwi_32924. (TASK-1135, GH #665)

- **Mosquitto dropped the gateway with "malformed packet" during bursts of small publishes.** `PubSubClient::beginPublish()` writes the fixed header and the topic to the socket with a single `write()` and reports only whether the full count went out. On ESP8266 that write returns a partial count once the lwIP send buffer stays full until the 5 second socket timeout (core 2.7.4, `ClientContext.h::_write_from_source` returns `_written` after `_is_timeout()`). All twelve call sites read the resulting `false` as "nothing happened" and returned with the connection still up, leaving half a PUBLISH header on the wire. The broker then read the next packet, a PINGREQ or the next publish, as the payload that header had promised. This needs no low heap, only send-buffer backpressure, which is why it hit two reporters on different routers and why reducing MQTT traffic did not cure it. TASK-769 closed the same desync for the payload half of a publish and left the header half open; the link is now dropped on a failed header write too, in `beginMqttPublish()` and in a shared helper for the nine discovery composers. A host test compiles the real PubSubClient against a short-writing client and reproduces both the defect and its absence after the fix. Note the disconnect is unconditional, because a partial write and a write that never started arrive as the same `false`: a needless reconnect costs one gap, a desynchronised stream costs every packet after it. (GH #682, TASK-1134)
...
- **An upload that was interrupted leaked a file handle.** A client disconnecting mid-body left the file open for the lifetime of the sketch, one handle per interruption, and LittleFS allows only a bounded number. A rapid-refresh storm during an upload could exhaust them and leave the filesystem unable to open anything at all. Interrupted uploads are now closed, and a stale handle is closed defensively when the next upload starts. (TASK-793, TASK-841, TASK-1039)
- **The main page kept streaming to browsers that had already gone away.** `index.html` is about 11 KB and was streamed without ever checking whether the client was still connected, and every write to a closed socket still walks the write path before failing, which costs loop time exactly when the device is under load. The stream now stops. The two low-memory refusals also carry `Retry-After`, so a refused client backs off instead of re-requesting immediately and holding the heap in the state that caused the refusal. Measured under a scripted storm: completed requests rose from 14 of 47 to 18 of 47 at six concurrent clients, with no crash and no reboot in any run. (TASK-793, TASK-843)

### Changed

- **Breaking: port 25238 (the legacy OTmonitor port) accepts two clients again, with one writer at a time.** v1.7.5 limited the port to one client, because two tools writing at the same time could mix their bytes into one broken command for the PIC, which left anyone running Home Assistant or Domoticz next to OTmonitor stuck on v1.7.4. What changes, and what can affect an existing setup:
  - Two clients can connect at the same time, for example the Home Assistant OpenTherm Gateway integration and OTmonitor. Both receive everything the PIC sends. A tool that was refused on v1.7.5 now gets in.
  - Only one client at a time can send to the PIC. It keeps that right until it has been quiet for 100 ms; the other client's command waits in the meantime and then goes through whole. A command can therefore be delayed by up to 100 ms after the other client's last byte. Commands can no longer get mixed up.
  - Nothing is changed on the way through. Every byte a client sends reaches the PIC exactly as sent, in both directions; the port stays fully transparent.
  - A PIC firmware upgrade from OTmonitor over this port is protected: once a client sends binary data it keeps the right to send until it has been quiet for 3 seconds, so the other client cannot interrupt the upgrade, and cannot send anything during it. Upgrading the PIC over the network remains discouraged; the PIC flash page of the web interface is the safer route.
  - A third connection is refused. If it comes from the address of a client that is already connected, it replaces that client instead, so a tool that crashed and reconnects gets its place back.
  - The Device Info page and `/api/v2/device/info` show how many clients are connected (`otgwstream_clients`), their addresses (`otgwstream_client_ip`) and the last address that was refused (`otgwstream_last_refused_ip`). If OTmonitor cannot connect, that tells you who holds the port.
  Requested by Schelte Bron and iandury_. (ADR-097, TASK-1167)

### Internal

- **Ten `String` uses removed from the OpenTherm path, and two stale ADR-judge matches cleared.** The `String` sites were the ones the ADR-049 audit flagged: on ESP8266 heap fragmentation is a stability concern rather than a style preference, so they are now `char[]` with the usual bounded helpers. Separately a dead BearSSL alias was dropped and a comment reworded, because both were matching the ADR enforcement patterns as prose and producing hits that described nothing real. Neither changes user-visible behaviour. (TASK-1141, TASK-1142, ADR-049)
