...
// heartbeat because these two are slow-moving intent, not per-cycle demand. (TASK-1061)
...
static void publishHvacMode(bool forcePublish)
{
  ...
  if (forcePublish || mode != mqttLastHvacMode || hvacHeartbeatDue(mqttLastHvacModeSent)) {
    ...
    // must retry on the next frame, not restart the 5-minute window. (TASK-1061)
...
static void publishHvacAction(bool forcePublish)
{
  ...
  if (forcePublish || action != mqttLastHvacAction || hvacHeartbeatDue(mqttLastHvacActionSent)) {
    ...
    // heartbeat stamp follows the same rule. (TASK-1061)
