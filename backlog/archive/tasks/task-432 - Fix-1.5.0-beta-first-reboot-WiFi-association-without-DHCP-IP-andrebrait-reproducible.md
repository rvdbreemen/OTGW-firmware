...
TASK-432 archived as obsolete/stale. The first-reboot DHCP regression described in this task was already resolved historically in the 1.5.x codebase before this task was filed. The dev worktree archived this task on 2026-05-06 (commit bf735582) with the same conclusion: networkStuff.ino already contained the fix, no new code work needed. The 1.5.x WiFi fixes (avoid re-binding TCP listeners, downgrade WiFiManager, remove wifi_station_dhcpc_start(), do not call WiFi.disconnect()) are all on the 1.5.x branch and not relevant to the 2.0.0 ESP32 line. No 2.0.0-specific action required.
