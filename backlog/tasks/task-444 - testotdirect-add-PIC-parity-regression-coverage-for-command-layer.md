...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 A curated PIC-parity command fixture exists in the repository with source comments pointing to `other-projects/otgw-6.6/gateway.asm`, local manuals, and OpenTherm v4.2 references.
- [ ] #2 The fixture or tests cover `MM=0` as valid MsgID 14 data `0x0000`, not a clear operation.
- [ ] #3 The fixture or tests cover `SW=<value>` as DHW MsgID 56 and confirm unsupported `TW=` is rejected or absent from supported command lists.
- [ ] #4 The fixture or tests cover `GW=0`, `GW=1`, and `GW=R` PIC-compatible semantics, plus any explicit OTDirect-only bypass alias if one is added.
- [ ] #5 The fixture or tests cover `MI=500` response/reporting units and `PR=N` reporting milliseconds.
- [ ] #6 The fixture or tests cover `SR=21:4,27` and `SR=22:7,234` decimal byte parsing and at least one invalid `SR=` form.
- [ ] #7 The fixture or tests cover `CS`/`C2` expiry expectations without requiring a real 60-second wait, using an injectable time source or a documented test shortcut if implemented.
- [ ] #8 The fixture or tests cover `TC` not writing MsgID 1 and distinguish `CS`, `TC`, and `TT` mappings.
- [ ] #9 External-client examples from `other-projects/otmonitor-6.6` are cited in comments or test descriptions for `SW`, `GW`, `SC`, and `SR` behavior.
- [ ] #10 The test approach does not require rewriting the OTDirect dispatcher and does not parse `gateway.asm` dynamically in production code.
- [ ] #11 The verification command or manual test steps are documented in the task final summary when implemented.
<!-- AC:END -->
