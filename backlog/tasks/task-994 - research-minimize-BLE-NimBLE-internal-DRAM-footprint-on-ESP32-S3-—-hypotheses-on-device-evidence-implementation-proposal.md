...
- [x] #3 Each executed hypothesis has on-device heap + ramp + BLE-functional evidence
- [x] #4 Implementation proposal written with alternatives and trade-offs
- [x] #5 Maintainer decision recorded (yes/no per option)
<!-- AC:END -->
...
Maintainer decision recorded (2026-07-06, in-session): Option A -- do nothing further. Confirms the recommendation already made in the proposal. No further hypothesis execution needed; the remaining ~35-40KB gap (corrected baseline, this session) is accepted as-is.
<!-- SECTION:NOTES:END -->

## Final Summary

<!-- SECTION:FINAL_SUMMARY:BEGIN -->
6 hypotheses documented, adversarially reviewed (2 overstated claims corrected), implementation proposal with 3 options written. Maintainer selected Option A: accept the current state, no further BLE-DRAM-reduction work. Task closed with the already-shipped NimBLE trim (hypothesis 1) as the only change, which already cut the original 64K gap down to ~35-40K.
