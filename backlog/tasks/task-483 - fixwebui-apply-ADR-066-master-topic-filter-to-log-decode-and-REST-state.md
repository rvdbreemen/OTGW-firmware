...
- [ ] #8 #8 PS=1 summary path (publishPSSummaryFieldValue in OTGW-Core.ino) gates sendMQTTData base-topic publish on bSlaveEchoesValue lookup for non-echo MsgIDs (Tr/TrSet/TrSetCH2/TSet/TsetCH2/MaxRelModLevelSetting and any other OT_WRITE/OT_RW MsgID with bSlaveEchoesValue=false in OTmap)
- [ ] #9 #9 PS=1 summary path skips updatePSSummaryFloatState/U16State for non-echo MsgIDs to keep OTcurrentSystemState consistent with the live-bus gate
- [ ] #10 #10 setMsgLastUpdated remains called regardless (cosmetic epoch tick consistent with live-bus path at OTGW-Core.ino:4034)
- [ ] #11 #11 Suppression emits a single DebugTln/DebugTf trace per gate hit so support can correlate with port-23 telnet logs
- [ ] #12 #12 New evaluate.py gate check_ps_summary_master_topic_gate prevents future PS=1 case additions from skipping the bSlaveEchoesValue lookup (ADR-080 conformance)
- [ ] #13 #13 ADR-066 amended (or follow-up ADR drafted) to document PS=1 inclusion explicitly
<!-- AC:END -->
