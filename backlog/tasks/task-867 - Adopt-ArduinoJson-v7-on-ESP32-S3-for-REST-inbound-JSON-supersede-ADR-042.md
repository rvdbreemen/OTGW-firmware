...
AC#8 UPDATE after a controlled hardware A/B (pre-migration alpha.204 vs migrated alpha.211, same device/conditions). HEAP AXIS (= the literal AC#8 criterion: ADR-089 tiers + fragmentation): CLEAN on the migrated build - no leak, 0 tier entries, fragmentation peaks under churn then recovers, idle returns to ~44% baseline. No heap regression. HOWEVER the A/B surfaced a CPU-axis regression the migration DID cause: under an unthrottled 8-worker flood the migrated build reboots on the IDF Task-Watchdog (bootcount 1->3 in 1 min) whereas the pre-migration build survives the SAME flood and even a 16-worker/2-min flood with zero reboots. Realistic throttled load (4 workers @1s, ~3.2 req/s) passes cleanly on BOTH builds (0 reboots, 100% success) -> the penalty is abuse-only, real-world dashboard load is safe. So the migration is safe to ship for normal use but it measurably lowered the flood/abuse TWDT margin (heavier two-pass build+serialize + heap-pool churn holding core 1 / starving loopTask). This is tracked in TASK-883 with the recommended fix (move async_tcp to core 0, amend ADR-139, which also fixes the pre-existing TASK-879 web-dead TWDT). AC#8's heap criterion is MET; the flood-TWDT is a separate CPU-starvation finding flagged for the maintainer's ship/hold decision. AC#8 stays In Review.
<!-- SECTION:NOTES:END -->
