...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 `MM=0` on OTDirect enqueues/sends WRITE_DATA MsgID 14 with f8.8 value `0x0000` instead of clearing the write cache.
...
- [ ] #3 `MM=1` through `MM=100` continue to enqueue MsgID 14 with correct f8.8 values.
...
- [ ] #5 SAT OPV calibration and PWM paths that send `MM=0` continue to use the same `addCommandToQueue()` call sites; no SAT control-loop rewrite is introduced.
...
- [ ] #7 PIC build behavior is untouched.
<!-- AC:END -->
