...
- [ ] #2 ADR-098 reviewed and approved by user; status flipped from Proposed to Accepted before implementation begins
- [ ] #3 ADR-097 status line updated to 'Superseded by ADR-098, YYYY-MM-DD'; body of ADR-097 unchanged (immutability protocol)
- [ ] #4 buildSensorDiscoveryTopic in mqtt_configuratie.cpp on the 2.0.0 branch changed: source-variant branch format string is '%s/sensor/%s/%s_%s/config' (sibling-suffix)
- [ ] #5 Canonical-branch format string is unchanged
- [ ] #6 ADR-097 Enforcement carve-out comment is removed or updated to reference ADR-098
- [ ] #7 ADR-098 Enforcement block forbid_pattern matches the OLD nested format and would catch any regression
- [ ] #8 Build for ESP8266 target exits 0 with no new warnings
