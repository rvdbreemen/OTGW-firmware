...
- [x] #3 FS OTA via bare curl (no size=) succeeds; version.hash flips; verified on device
...
FS-OTA over the API fixed and verified on-device (combo .64, alpha.311+9d29eea). Root causes: the size guard + Update.begin used LittleFS.totalBytes() (usable < partition) and _parseUploadTotalSize fell back to request->contentLength() (multipart body > partition), so a full-partition mklittlefs image was rejected 'filesystem image too large' from a bare curl. Fix (OTGW-ModUpdateServer-esp32.h): size FS begin+guard from esp_partition_find_first(DATA,SPIFFS)->size; _parseUploadTotalSize returns 0 when size= absent (no contentLength fallback). Mirrors 1.x ESP8266 (_FS_end - _FS_start, size-arg-only). Result: FS OTA works from a bare curl (no size=), not only the browser form. Verified: POST /update?cmd=100 with no size param -> HTTP 200 'Flashing successful!', version.hash flipped, board healthy. Builds green (combo/classic/otgw32), evaluate clean, adversarial review 0 defects (1 comment nit fixed). Commit 9d29eea8.
