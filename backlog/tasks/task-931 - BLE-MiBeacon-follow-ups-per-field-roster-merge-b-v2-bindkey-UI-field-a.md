...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 Per-field merge: onResult inits temp/hum to NAN and batt to 0xFF; the runtime update writes a field to _bleRuntime[slot] ONLY when it is non-sentinel; temperature is never overwritten with 0 by a humidity-only or battery-only advert
- [ ] #2 parseBLEMiBeaconFormat registers a frame on ANY decoded field (temp OR humidity OR battery), not temperature-only; ATC and BTHome parser return semantics are unchanged; a humidity-only stock-Mijia advert now updates the slot humidity while keeping the last temperature
- [ ] #3 v2 BLE roster card shows per-slot has_key state and offers a minimal bindkey input that POSTs {mac,key} to /api/v2/sat/ble/bindkey; the key is never rendered back (write-only); empty input clears
- [ ] #4 Builds green esp32 + esp32-classic; evaluate.py --quick no new failures; known-answer test scripts/test_mibeacon_decrypt.py still passes
