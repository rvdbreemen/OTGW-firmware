...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 3B-H1: `data/index.js` `iBleInterval` tooltip on lines 5833 and 5948 reads `publish/state-update cadence` (not `scan rate`)
- [ ] #2 3B-H2: `docs/api/MQTT.md` documents the per-MAC topic family `OTGW/sat/ble/<mac>/{temp,rh,bat,rssi}` plus the `<HaPrefix>/sensor/<uniqueId>_ble_<mac>_<kind>/config` discovery topic with payload examples and retain semantics
- [ ] #3 3B-H3: `CHANGELOG.md` `[Unreleased]` section lists the 8 session commits with user-visible behaviour changes called out
- [ ] #4 3C-1: ADR-090 sub-rule 4 amended with FreeRTOS cross-task case + TASK-497 reference
- [ ] #5 3C-2: ADR-092 amended to reflect continuous-scan + iBleInterval semantics shift; TASK-494 referenced
- [ ] #6 3C-3: ADR-068 amended (or new ADR-093 created) for `OT_CMD_QUEUE_SIZE = 12` correction + coalesce-by-MsgID semantics + queue-is-the-channel principle
- [ ] #7 ESP32 build clean (no firmware behaviour change)
- [ ] #8 ESP8266 build clean
- [ ] #9 python evaluate.py --quick zero new violations
- [ ] #10 All amended ADRs retain Status=Accepted with the four verification gates discharged
- [ ] #11 TASK-494 description corrected (or annotated) to reference ADR-068 instead of ADR-064/ADR-087
<!-- AC:END -->
