...
- [x] #3 HA OtCore device suffix/short-name (-pic/-ot-direct) threaded through HaDiscoveryContext from runtime eMode (avoids orphaned HA device)
- [x] #4 Fixed-board discovery JSON byte-identical before/after
- [x] #5 esp32-combo build green; evaluator no new failures
- [x] #6 Field-validated: HA shows correct device identity per mode
<!-- AC:END -->
...
AC3 implementation (alpha.177, commit 92005aad): added otCoreSuffix/otCoreName to HaDiscoveryContext, populated runtime in buildDiscoveryContext() under HAS_RUNTIME_HW_DETECT, threaded ctx into haDeviceSuffix() at all 12 call sites + haDeviceShortName(). buildDiscoveryDeviceBlock() snprintf path uses runtime suffix on combo. topoDeviceName() compile-time intentionally (mode-switch orphaning = accepted manual-cleanup, ADR-124). Fixed boards: byte-identical by construction. AC6 field validation pending hardware.
<!-- SECTION:NOTES:END -->
