...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 buildCurveOption() in data/sat.js:470 is read end-to-end and the precise reason refCoeffs (c=0.5..5.0) are not in the rendered series array is identified in the implementation notes
- [ ] #2 The 10 grey reference curves render in the Heating Curve (Stooklijn) panel on a fresh build with SAT enabled, regardless of live data availability (cold device, no boiler attached)
- [ ] #3 The active coefficient curve (orange) renders alongside the grey references; the active dot continues to render at the current outside / flow point as it does today on alpha.8
