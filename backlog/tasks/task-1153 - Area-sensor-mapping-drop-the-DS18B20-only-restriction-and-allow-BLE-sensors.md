...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 The panel is titled 'Area Sensor Mapping' with no DS18B20 in title or hint, in the classic UI and in v2
...
- [ ] #3 The persisted area-sensor field accepts both a 16-hex Dallas address and an 18-byte BLE MAC; existing Dallas-only settings.json files load unchanged after upgrade
- [ ] #4 GET and PATCH /api/v2/sat/sensor-areas accept and return both address forms; invalid values still 400; the GET response buffer is sized for the widest case
- [ ] #5 The area dropdown is populated from all discovered sensors (Dallas via /api/v2/sensors, BLE via /api/v2/sat/ble/discovery), each showing its user label where one exists
- [ ] #6 The v2 UI gets the same picker instead of the four raw free-text satsensorarea0..3 inputs
- [ ] #7 Exactly one in-firmware writer per source: the Dallas poll loop and the BLE update path each call satSetAreaTemp() only for their own mapped areas
- [ ] #8 Frontend stays within the N<=2 in-flight cap (ADR-165) while fetching the two discovery lists plus the current mappings
- [ ] #9 An ADR (or ADR-051 amendment) records widening sSensorArea to a multi-source sensor reference, including the migration
- [ ] #10 python build.py green for the ESP32 targets; python evaluate.py --quick shows no new failures
<!-- AC:END -->
