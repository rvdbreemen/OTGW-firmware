...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 Reproduce: flash esp32-combo to a PIC-less OTGW32, confirm WiFi up (ping) but port 80/23 refused; capture where setup() stalls (telnet unavailable -> use USB-CDC debug or a temporary Serial trace gated for combo).
- [x] #2 Root-cause the boot-detect: confirm GPIO44 floating-read false-PIC-detect (or find the real stall point); document the exact code path that blocks before server.begin().
- [x] #3 Add a boot-mode force/clear (per TASK-865.17): the combo must reach server.begin() and serve web+telnet on a PIC-less OTGW32, with a way to force OTDirect when PIC detection is ambiguous.
- [x] #4 Field-validate: esp32-combo on a PIC-less OTGW32 serves /api/v2 and telnet; AND on a real Classic-with-PIC it still boot-detects PIC correctly.
<!-- AC:END -->
...
2026-07-09 FIXED + verified on-device (OTGW32 .39 / COM4, genuine PIC-less, flashed esp32-COMBO alpha.341). AC#1: the original stall is GONE — combo boots and serves. AC#2: root cause resolved by the boot-detect rework (TASK-947/949/ADR-160 + today's TASK-1031 fix): a no-PIC result no longer blocks waiting on a phantom PIC — detectPIC() retries then, finding no ETX, the flow proceeds to initOTDirect() (iBoardMode stays 0) and reaches server.begin(). AC#3: combo on the PIC-less OTGW32 serves BOTH web+telnet — HTTP :80 => 200 (/api/v2/device/info), TELNET :23 => OPEN; board='Nodoshop OTGW Combo (ESP32-S3)', hardware_type=otgw32, hardwaremode=OT-Direct (NO false-PIC-detect), stable (uptime climbing, reboots=1 no loop, concurrent 4x ramp served). AC#4: field-validated both ways — PIC-less OTGW32 serves (this test) AND a real Classic-with-PIC (the S3 Mini Pro .74) boot-detects PIC correctly (hardwaremode=PIC, pic16f1847, verified earlier this session). The maintainer directive to ship esp32-combo as the single build is unblocked.
<!-- SECTION:NOTES:END -->

## Final Summary

<!-- SECTION:FINAL_SUMMARY:BEGIN -->
esp32-combo now serves web+telnet on a PIC-less OTGW32 (was: setup() stalled before server.begin() on a false-PIC-detect). Fixed by the boot-detect rework — a no-PIC result proceeds to OT-Direct instead of blocking on a phantom PIC. Verified on-device: combo on the OTGW32 .39 serves HTTP :80 (200) + telnet :23, detects OT-Direct correctly, stable; and a real Classic-with-PIC still boot-detects PIC. Unblocks combo-as-single-build.
