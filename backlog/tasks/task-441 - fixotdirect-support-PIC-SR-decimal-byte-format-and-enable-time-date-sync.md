...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 OTDirect accepts PIC-compatible `SR=<msgId>:<lowByte>` with decimal values and stores/sends a response override whose high byte is `0x00` and low byte is `<lowByte>`.
- [ ] #2 OTDirect accepts PIC-compatible `SR=<msgId>:<highByte>,<lowByte>` with decimal byte values 0 through 255 and combines them as `(highByte << 8) | lowByte`.
- [ ] #3 `SR=21:4,27` produces OpenTherm response data `0x041B`; `SR=22:7,234` produces `0x07EA` for year 2026.
- [ ] #4 Out-of-range byte values, malformed comma syntax, missing message IDs, and unsupported message IDs are rejected without updating the response cache.
- [ ] #5 If the existing hex16 `SR=<msgId>:<hex16>` form is retained for backward compatibility, comma-containing PIC syntax takes precedence and documentation clearly identifies the PIC-compatible form as canonical.
- [ ] #6 `networkStuff.ino` time/date synchronization is enabled for OTDirect through `hasOTCommandInterface()` or an equivalent PIC-or-OTDirect predicate only after the parser is PIC-compatible.
- [ ] #7 `SC=` behavior remains unchanged except for being reachable on OTDirect through the existing command interface path.
