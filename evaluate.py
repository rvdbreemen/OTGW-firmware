...
class WorkspaceEvaluator:
    ...
    # ===== ADR-089 / ADR-121 GATES: RETIRED BY ADR-167 (TASK-1036) =====
    #
    # check_heap_tier_thresholds_ordered, check_heap_fragmentation_promotion,
    # check_heap_tier_entry_counters (ADR-089 sub-rules 1-3) and
    # check_per_consumer_heap_gate (ADR-121 Option B) lived here. ADR-167
    # retired the ESP8266-era heap tier machine and the per-consumer gating on
    # this ESP32-S3-only branch, so the code surface these gates parsed no
    # longer exists. Per ADR-080 a superseded binding rule must not keep a live
    # CI gate: the gates are removed, not left passing vacuously.
