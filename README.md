...
This release marks a significant milestone for the project. After years of development and testing, the REST API and MQTT interface have proven their stability and reliability. This release embodies the original vision for the firmware, bringing polished features including:

- **Live Data Streaming**: Watch OpenTherm messages arrive in real-time directly in your browser via WebSocket
...
- **Dark Theme**: Fully integrated dark theme with persistent toggle across all pages
- **Advanced Developer Tools**: New build system and automated flashing tools with comprehensive documentation

This release includes many stability and security improvements to make it significantly more robust than the 0.10.x series. See the [detailed release notes](#release-notes) below for the complete changelog.
...
- View a live OpenTherm message log stream with advanced controls (auto-scroll, timestamps, capture mode, auto-screenshot) and download logs (WebSocket-based).
- View real-time graphs of OpenTherm data with time window controls.
- Track firmware upload/flash progress with size validation during updates.
- Toggle a dark theme with per-browser persistence across all pages.
...
- InfluxDB client for direct logging to InfluxDB databases
- Additional live sensor cards in the Web UI with real-time value updates (currently OT message log and device info refresh every second)
...
|-|-|
| 1.0.0 | **Milestone Release**: This version marks a major milestone, representing the complete vision of the firmware with a stable API and robust MQTT integration. After years of development and testing, the firmware has proven its stability and reliability for Home Assistant integration and OpenTherm monitoring.<br><br>**Major Features**:<br>- **Live OpenTherm Logging**: Real-time streaming of OpenTherm messages in the Web UI via WebSockets with comprehensive log viewer controls (auto-scroll, timestamps, capture mode, auto-screenshot)<br>- **Enhanced Firmware Updates**: Improved UI for flashing ESP and PIC firmware with live progress bars, validation, and global state management<br>- **Dark Theme**: Fully integrated dark theme with persistent toggle across all UI pages<br>- **Advanced Graphing**: Enhanced graph functionality with extended data buffer support for higher message rates and time window controls<br>- **NTP Control**: New setting `NTPsendtime` to optionally disable time sync to the thermostat<br>- **Improved Developer Experience**: New build system (`build.py`) and automated flash script (`flash_esp.py`) with comprehensive documentation (BUILD.md, FLASH_GUIDE.md)<br><br>**UI & UX Improvements**:<br>- Refactored DevInfo layout from grid to table format for improved readability and responsiveness<br>- Allow Type 0 messages (Master/Slave Status) in Web UI with appropriate labeling<br>- Enhanced log line formatting with direction field for improved trace readability<br>- Auto-refresh for device time and OT monitor data (1-second intervals)<br><br>**Performance & Architecture**:<br>- Refactored WebSocket logging architecture with direct broadcasting (removed JSON queue serialization)<br>- Implemented backpressure handling in WebSocket queue and increased message processing capacity<br>- Conditional WebSocket support with fallback for numeric value parsing<br>- Improved data validation in processLine function with enhanced error handling<br>- Flash mode management to pause background tasks during firmware updates<br><br>**Stability & Security**:<br>- **Security**: HTTP method validation for all API endpoints, masked password fields in UI, safe string handling (strlcpy/snprintf) throughout codebase to prevent buffer overflows<br>- **Connectivity**: Rewritten Wi-Fi connection logic with better watchdog handling and reliability<br>- **Reliability**: Numerous fixes for buffer overflows, memory leaks, and serial communication stability<br>- **Code Quality**: Extensive improvements to memory management (PROGMEM usage) and code structure<br>- Enhanced filesystem flashing with detailed debug messages<br><br>**Documentation**:<br>- Added comprehensive BUILD.md guide for local development<br>- Added FLASH_GUIDE.md with platform-independent flashing instructions<br>- Added EVALUATION.md documenting code quality framework<br>- Added Copilot instructions for AI-assisted development<br><br>A huge thank you to everyone who helped test, develop, and support this project to reach this milestone! Special thanks to all contributors, testers, and the community for their extensive feedback throughout the release candidate phase. |
