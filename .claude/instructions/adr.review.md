...
When you (Claude Code) are reviewing a PR or a diff, run these six checks for ADR compliance. The canonical guide is `docs/adr/README.md`; the comprehensive skill is `.claude/skills/adr/SKILL.md`.
...
- **Interfaces and contracts**: API behaviour, MQTT topics, REST endpoints, breaking changes
...
- Suggest the `.claude/agents/adr-generator.md` subagent for fast scaffolding.
...
- Check ADR-specific compliance (the `CLAUDE.md` "Binding ADRs" section lists which ADRs have CI gates and which are pattern-level)
...
Apply the four named gates from `.claude/skills/adr/SKILL.md`:
...
- Suggest a fix.
- Reference an existing ADR in this repo as a positive example (e.g. ADR-077 has strong evidence; ADR-084 has strong consistency).
...
- Request a remediation plan: an incremental cleanup, a tech-debt backlog task with a target version, or an explicit "out of scope for this PR" note
- Do not block the ADR adoption itself due to legacy issues
- Ensure the *new* code in this PR complies

## Review-comment templates

Match the project's existing review-comment style. Examples:
...
The .claude/agents/adr-generator subagent can scaffold this. See
.claude/skills/adr/SKILL.md for the template.
```
...
2. "Memory constraint requires this" (line M): cite where (datasheet,
   measured, SDK doc) and how much.

ADR-077 in this repo is a good positive reference for the Evidence gate.
```
