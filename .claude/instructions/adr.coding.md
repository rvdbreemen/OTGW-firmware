...
When you (Claude Code) are about to implement or change code, run through these rules **before** writing.

## Before implementing

- **Check for existing ADRs**: scan `docs/adr/` for an ADR that governs the area you are about to touch. The fastest path: read `docs/adr/README.md` index by topic, then open the relevant ADRs.
- **Assess architectural significance**: does the change affect any of the following?
  ...
  - Interfaces and contracts: public APIs, MQTT topics, REST endpoints, breaking changes
...
- **Use the canonical guide**: `docs/adr/README.md` is the source of truth for ADR format, lifecycle, and superseding rules. `.claude/skills/adr/SKILL.md` is the comprehensive how-to.

## Creating a new ADR

If no ADR exists and the change is architecturally significant:

1. **Hand the work to the ADR generator**: invoke the `adr-generator` subagent (`.claude/agents/adr-generator.md`). Provide it with the title, decision, alternatives, constraints, and any related ADRs. The agent writes a fully-formed ADR file at `docs/adr/ADR-XXX-title.md`.
...
3. **Update the old ADR's Status line only**: `Superseded by ADR-YYY`. Do not add a Status note that contains the rationale; rationale lives in the new ADR.
4. **Cross-link** in both ADRs' `## Related Decisions` sections.

## Amending (vs superseding)

Some ADRs have a stable contract but a narrow scope-change. The project distinguishes:
...
If you are unsure, default to creating an ADR. The cost of writing one is 15 to 30 minutes; the cost of re-deriving the rationale 6 months from now is hours. (See the Anti-Rationalization Guards table in `.claude/skills/adr/SKILL.md`.)
