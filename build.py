...
TARGETS["esp32-combo"] = {
    **TARGETS["esp32"],
    ...
    # The combo links BOTH OT engines (PIC + OTDirect) so its binary overflowed
...
def generate_v2_css_bundle(project_dir):
    """TASK-989: concatenate ds-tokens.css + v2.css into a single v2-bundle.css so
    the v2 Web UI needs one CSS request instead of two on first paint. Generated
    at build time (single source of truth stays ds-tokens.css/v2.css; this file
    is regenerated every build, same pattern as version.hash) rather than
    hand-duplicated, so the two source files never drift out of sync with the
    bundle. ds-tokens.css defines the CSS custom properties v2.css consumes, so
    concatenation order matters: tokens first, then v2's rules.

    ds-tokens.css is NOT deleted or touched — it is still served standalone for
    design.html, index.html, and FSexplorer.html, which don't get this bundle.

    The 3 @font-face blocks are stripped from the bundle (field bug, esp32-otgw32,
    2026-07-06): the browser fetches every @font-face src the instant the CSS
    parses, in an uncontrolled native parallel burst outside any JS retry logic.
    3 simultaneous font fetches exceed WEB_FILE_MAX_INFLIGHT=2 (ADR-165), and
    unlike the CSS/JS retry-loader, a failed font fetch has no browser-native
    retry -- it fails permanently until a full page reload. v2.html loads the
    same 3 fonts instead via the FontFace() JS API, one at a time with retry,
    so peak concurrent font requests stays at 1. ds-tokens.css itself keeps its
    @font-face blocks unchanged for the other three pages, which are not part of
    this bug report.
    """
    import re as _re

    data_dir = config.DATA_DIR
    tokens_path = data_dir / "ds-tokens.css"
    v2css_path = data_dir / "v2.css"
    bundle_path = data_dir / "v2-bundle.css"

    tokens_content = tokens_path.read_text(encoding="utf-8")
    v2css_content = v2css_path.read_text(encoding="utf-8")

    tokens_content_no_fonts = _re.sub(r"@font-face\s*\{[^}]*\}\s*", "", tokens_content)

    header = (
        "/* AUTO-GENERATED by build.py (generate_v2_css_bundle, TASK-989). "
        "Do not edit directly -- edit ds-tokens.css / v2.css and rebuild.\n"
        "   @font-face blocks stripped -- v2.html loads fonts via FontFace() "
        "instead (sequential, retryable; see v2.html for why). */\n"
    )
    bundle_path.write_text(header + tokens_content_no_fonts + "\n" + v2css_content, encoding="utf-8")
    print_info(f"Generated v2-bundle.css ({tokens_path.name} [no @font-face] + {v2css_path.name}, "
...
def archive_build_artifacts(project_dir, semver):
    ...
    .elf for any shipped or bisect build so an ESP32 panic backtrace can be decoded
    with addr2line long after build/ has been overwritten. .bin
    (firmware/filesystem/merged) and .zip (flash bundles) are archived alongside so
    a build is fully reproducible from its zip.
    """
    ...
    # Sanitize semver for use as a file name (defensive; normally safe).
...
def _build_readme_en(target, tcfg, merged_full_name, upgrade_bin_name, semver):
    ...
    return (
        ...
        f"     .ino.bin (firmware) and .littlefs.bin (filesystem) as\n"
        f"     individual downloadable assets. Upload them via\n"
        f"     Web UI -> Update. This archive (with the merged binaries\n"
        f"     and flash scripts) is for the initial USB flash and\n"
        f"     recovery scenarios only.\n"
        f"\n"
...
def _build_readme_nl(target, tcfg, merged_full_name, upgrade_bin_name, semver):
    ...
    return (
        ...
        f"     .ino.bin (firmware) en .littlefs.bin (filesystem) als\n"
        f"     aparte downloadbare assets. Upload die via Web UI ->\n"
        f"     Update. Dit archief (met de merged-binaries en flash-\n"
        f"     scripts) is bedoeld voor de eerste USB-flash en\n"
        f"     herstelscenario's.\n"
        f"\n"
...
Examples:
  ...
  build --arduino-cli                        # Use legacy arduino-cli backend
    ...
    # TASK-989: regenerate the v2 CSS bundle (ds-tokens.css + v2.css -> v2-bundle.css)
    # before packing the filesystem, so it's always current with the source files.
    generate_v2_css_bundle(project_dir)
