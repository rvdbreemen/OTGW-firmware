...
- `.gitignore` — Git ignore rules (~429 tok)
- `.gitmodules` (~64 tok)
- `.mcp.json` (~7 tok)
- `AGENTS.md` — OTGW-firmware: Codex Agent Instructions (~9522 tok)
- `AUTHORS` — Authors (~19 tok)
- `build_design_system_pass.log` — Declares char (~13387 tok)
- `build_task435_followupA.log` — Declares char (~17733 tok)
- `build_task450_retry.log` — Declares char (~22712 tok)
...
- `partitions_otgw_esp32.csv` — OTGW-firmware ESP32-S3 partition table — single app (no OTA), 4MB flash (~214 tok)
- `platformio.ini` — Declares names (~2319 tok)
- `README.md` — Project documentation (~4958 tok)
- `RELEASE_GITHUB_1.4.1.md` — Bug fixes (~1278 tok)
- `RELEASE_NOTES_1.4.1.md` — OTGW-firmware v1.4.1 Release Notes (~3463 tok)
...
- `backlog_discord.md` — /backlog_discord -- Respond to backlog commands and @bot mentions from Discord (~3862 tok)
- `check_otgw_issues.md` — /check_otgw_issues — Monitor Discord, GitHub and Tweakers for user-reported issues (~2962 tok)
...
- `SKILL.md` — /release - OTGW-firmware Release Skill (~2462 tok)
...
- `00-scope.md` — Review Scope (~953 tok)
- `01-quality-architecture.md` — Phase 1: Code Quality & Architecture Review (~1564 tok)
- `02-security-performance.md` — Phase 2: Security & Performance Review (~1679 tok)
- `03-testing-documentation.md` — Phase 3: Testing, Documentation & ADR Audit (~1684 tok)
- `04-best-practices.md` — Phase 4: Best Practices & Standards (~936 tok)
- `05-final-report.md` — Comprehensive Code Review Report (~2039 tok)
- `06-followup-plan.md` — Follow-up Plan — Comprehensive Review Mediums & Lows (~2541 tok)
- `07-closure-summary.md` — Comprehensive Review Closure Summary (~4243 tok)
- `phase1a-code-quality.md` — Phase 1A: Code Quality Review (~4959 tok)
- `phase1b-architecture.md` — Phase 1B: Architecture & Design Review (~4834 tok)
- `phase2a-security.md` — Phase 2A: Security Audit (~3704 tok)
- `phase2b-performance.md` — Phase 2B: Performance & Scalability Review (~3775 tok)
- `phase3a-testing.md` — Phase 3A: Test Coverage & Quality Review (~5059 tok)
- `phase3b-documentation.md` — Phase 3B: Documentation & API Review (~5580 tok)
- `phase3c-adr-audit.md` — Phase 3C: ADR Audit (~4277 tok)
- `phase4a-best-practices.md` — Phase 4A: Framework & Language Best Practices (~4145 tok)
- `phase4b-cicd.md` — Phase 4B: CI/CD & DevOps Practices (~4752 tok)
- `state.json` (~252 tok)
...
- `copilot-instructions.md` — GitHub Copilot Instructions for OTGW-firmware (~15613 tok)
...
- `build.yml` — CI: PlatformIO firmware build (~443 tok)
- `claude-code-review.yml` — /*.ts" (~422 tok)
- `claude.yml` — CI: Claude Code (~554 tok)
- `dependency-scan.yml` — CI: PlatformIO dependency scan (~849 tok)
- `evaluate.yml` — CI: evaluate.py CI gates (~263 tok)
- `opentherm-v42-spec-audit.yml` — CI: OpenTherm v4.2 Spec Audit (~311 tok)
- `trigger-copilot-agent.yml` — CI: Trigger Copilot Coding Agent (~1391 tok)

## .pio/libdeps/esp32/

- `integrity.dat` (~92 tok)
