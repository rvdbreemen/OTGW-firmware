...
- **Single-flight API calls (NEVER burst).** Fire at most ONE `/api` request at a time. No `Promise.all` / `.map()` over `fetch`, no parallel request fans — always serialize with a sequential promise chain (`kv.reduce((c,p)=>c.then(()=>post(...)), Promise.resolve())`) so exactly one request is in flight. The device's REST backpressure gate (`restAPI.ino`, TASK-884) caps concurrent `/api` requests at 4 and tightens toward 1 under heap pressure; a burst gets a cheap 503, which surfaces as silent partial saves (the SAT onboarding "Could not save the settings, SAT was not enabled" bug, TASK-1014). This applies to GETs too, not just POSTs. Note: `post()` fires the fetch eagerly when called, so serialize by deferring the *call* (build `[name,value]` pairs, call `post` one at a time), not by awaiting a pre-built array of already-launched promises.
